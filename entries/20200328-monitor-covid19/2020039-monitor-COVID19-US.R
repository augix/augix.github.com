...
col18 = c('#e6194b', '#3cb44b', '#4363d8', '#f58231', '#911eb4', 
	...
	'#808080', '#000000')
col18=adjustcolor( col18, alpha.f = 0.5)
...
pdf(paste('f5d_China_vs_US_growth1',Sys.Date(),'pdf',sep='.'),w=10,h=30)
main='China vs. US'
n=nrow(cum)
...
pdf(paste('f5e_China_vs_US_growth2',Sys.Date(),'pdf',sep='.'),w=5,h=5)
main='China vs. US'
plot.points=F
adjust.label=F
...
# death
US.death = records2counts(dat =nyt, count_type= 'deaths', level= 'state')
US.death.cum = US.death$cum
US = US.death.cum
China.death = records2counts(dat =china, count_type = 'province_deadCount', level= 'provinceEnglishName')
China.death.cum = China.death$cum
China = China.death.cum
overlapped.days = intersect(colnames(China),colnames(US))
combined = rbind(China[,overlapped.days],US[,overlapped.days])
cum=combined
col = c(rep('gray',nrow(China)),rep(col18,nrow(US)))

pdf(paste('f6a_China_vs_US_death_growth_aligned',Sys.Date(),'pdf',sep='.'),w=5,h=5)
main='China vs. US'
plot.points=F
adjust.label=F
n1=sum(China[, overlapped.days[length(overlapped.days)]]>=20)
n2=sum(US[, overlapped.days[length(overlapped.days)]]>=20)
col = c(rep('gray',n1),rep(col18,n2))
layout(matrix(c(rep(1,16),2:(n1+n2+1)), nrow=1, ncol=1, byrow = TRUE))
cum2matplot.aligned(cum, n=n1+n2, align_n=20, day1='01-22')
dev.off()
